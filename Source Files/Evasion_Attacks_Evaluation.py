...
# Evaluation against evasion (FSGM) attack
# Load the fine-tuned model
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def evaluate_model_on_adversarial_examples(model, test_loader, attack):
    ...
    for x, y in test_loader:
        ...
        # Apply FGSM attack
        x_adv = attack(model, x, y, epsilon=0.01)
...
# Evaluate on FGSM adversarial examples only
fgsm_accuracy, fgsm_confidence, fgsm_asr = evaluate_model_on_adversarial_examples(model, test_loader, fgsm_attack)

print(f"Accuracy on FGSM adversarial examples: {fgsm_accuracy:.2f}% | Average Confidence: {fgsm_confidence:.4f} | Attack Success Rate: {fgsm_asr:.4f}")
